...
# signals = {"Lane 1":[17,18,19],"Lane 2":[20,21,22],"Lane 3":[23,24,25],"Lane 4":[26,27,28]}
signals = {"Lane 1":[4,27,22],"Lane 2":[23,24,25],"Lane 3":[6,13,26],"Lane 4":[5,12,7]}

for lane_signals in signals.values():
    for pin in lane_signals:
...
try:
    ...
    # inputWidth, inputHeight = 320, 320
    ...
    weightsPath = os.path.sep.join(["yolo-coco", "yolov4-tiny.weights"])
    configPath = os.path.sep.join(["yolo-coco", "yolov4-tiny.cfg"])
    ...
    def blink(lane):
        ...
        while time.time() < end_time:
            GPIO.output(signals[lane][0],GPIO.LOW)
            time.sleep(0.25)
            GPIO.output(signals[lane][0],GPIO.HIGH)
            time.sleep(0.25)

    def signal(lane,sigTime,flag):
        # capture_object = cap
        if(flag==1):
            GPIO.output(signals[lane][2],GPIO.LOW)
            GPIO.output(signals[lane][0],GPIO.HIGH)
            print(sigTime)
            # time.sleep(sigTime)
            time.sleep(sigTime-5)
            blink(lane)
            GPIO.output(signals[lane][0],GPIO.LOW)
        
        elif(flag==2):
            GPIO.output(signals[lane][1],GPIO.HIGH)
            print(sigTime)
            time.sleep(sigTime)
            GPIO.output(signals[lane][1],GPIO.LOW)
            
    def run_traffic_light_system(lane):
        print("Starting ...")
        time.sleep(3)
        GPIO.output(signals["Lane 1"][2],GPIO.HIGH)
        GPIO.output(signals["Lane 2"][2],GPIO.HIGH)
        GPIO.output(signals["Lane 3"][2],GPIO.HIGH)
        GPIO.output(signals["Lane 4"][2],GPIO.HIGH)
        ...
        while True:
            ...
            if vehicle_count == None:
                new_greentime = green_times[next_lane]
            else:
                new_greentime = adjust_green_time(vehicle_count)
            ...
            print("----------------------------------------")
            GPIO.output(signals[lane][2],GPIO.HIGH)
            # print(lane, "-> RED") 
            print("----------------------------------------")
